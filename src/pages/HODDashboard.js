            ...
            {activeTab === 'syllabus' && (<div className={styles.sectionContainer}>
                    ...
                    <div style={{ padding: '1.5rem' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem', marginTop: '1rem' }}>
                            <div className={styles.formGroup}>
                                ...
                                <input className={styles.input} id="newSubjectInstructor" placeholder="e.g. Dr. Smith" defaultValue={editingSubject?.instructorName || ''} key={editingSubject ? `edit-inst-${editingSubject.id}` : 'add-inst'} />
                            </div>
                            <div style={{ display: 'flex', alignItems: 'flex-end', gap: '0.5rem' }}>
                                ...
                                <button className={styles.primaryBtn} style={{ flex: editingSubject ? 2 : 1, width: '100%', justifyContent: 'center', padding: '0.65rem' }} onClick={async () => {
                                    ...
                                    try {
                                        const token = user?.token;
                                        const headers = { 'Content-Type': 'application/json', ...(token ? { 'Authorization': `Bearer ${token}` } : {}) };
                                        const payload = { name, code, department: selectedDept, semester: parseInt(semester), credits: 0, instructorName };
                ...
                <div className={styles.card} style={{ marginTop: '1.5rem' }}>
                    ...
                    <div className={styles.tableWrapper}>
                        <table className={styles.table}>
                            <thead><tr><th>Name</th><th>Code</th><th>Semester</th><th>Instructor</th><th>Action</th></tr></thead>
                            <tbody>
